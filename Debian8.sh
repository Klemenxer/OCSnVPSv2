...
sed -i 's|export KEY_PROVINCE="CA"|export KEY_PROVINCE="Zambales"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_CITY="SanFrancisco"|export KEY_CITY="Castillejos"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_ORG="Fort-Funston"|export KEY_ORG="LiteVPN"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_EMAIL="me@myhost.mydomain"|export KEY_EMAIL="dev.klemenxer@gmail.com"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_OU="MyOrganizationalUnit"|export KEY_OU="IT"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_NAME="EasyRSA"|export KEY_NAME="litevpn"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_OU=changeme|export KEY_OU=litevpn|' /etc/openvpn/easy-rsa/vars
...
# setting banner
rm /etc/issue.net
wget -O /etc/issue.net "https://raw.githubusercontent.com/Klemenxer/OCSnVPSv2/master/issue.net"
...
echo "               Created By Joshua Rivera(github.com/klemenxer)                   "
echo "                                Modified by Klemenxer                           "
echo "--------------------------------------------------------------------------------"
